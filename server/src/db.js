 ...
 *** Opening a DB connection is expensive. A Pool keeps a small set of connections  ready to use and reuses them for each query, which is faster and scalable.
 *** Who uses this? Your model files (e.g., patients.model.js) Routes call models; models call query(); query() uses the Pool under the hood.
 * Config: - Reads the connection string from process.env.DATABASE_URL (set in .env).*/
...
const pool = new Pool({                                   // ③ Create one pool for the entire process (module singleton).
