//***The patients router defines what URLs exist and what they do.rc/routes/patients.routes.js
...
//*** GET /api/patients?page=&pageSize=&q=  -> List patients with pagination + optional search.
...
//*** POST /api/patients  -> Create a new patient (server-side validation via Zod).  //***Zod is a validation library for JavaScript that Catch errors early
r.post('/', validate(patientCreateSchema), asyncH(async (req, res) => {              //asyncH() to catch async errors cleanly,
...
// ***PATCH /api/patients/:id  -> Partially update a patient. Only provided fields are changed.
r.patch('/:id', validate(patientUpdateSchema), asyncH(async (req, res) => {            //validate(schema) to run Zod validation on the request body
...
// DELETE /api/patients/:id  -> Remove a patient by id.
