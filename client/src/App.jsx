/*** Defines the top-level layout (header/nav) and the client-side routes. Includes a small "Health" page that calls the backend /health and prints the server time.
 *
 * How it connects: Uses the shared http() helper for API calls elsewhere (Patients pages).
...
function Health() {
  /**Health page: Calls the backend /health endpoint and shows the JSON result. Useful to quickly verify that frontend ↔ backend are connected.*/
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/health")
      ...
      // The extra .catch/.finally below duplicates the ones above. It doesn't break anything, but it's redundant (can be removed).
