/**
 * Purpose: A tiny, central helper for making HTTP calls from the frontend to your backend API. All UI code should use `http()` so your fetch logic and error handling stay consistent.

 * How it connects: Reads BASE URL from Vite env (client/.env: VITE_API_BASE) , Components call `http('/patients')`, `http('/patients/123', { method: 'PATCH', ... })`, etc. */
 ...
 *
Behavior: Merges your options with a default JSON Content-Type header. Throws a readable Error if `res.ok` is false (4xx/5xx) on on non-OK HTTP codes, Parses JSON automatically on success. */
